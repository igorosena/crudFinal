...
export class Function {
    byteOffset; // JUMPDEST byte offset
    opTags; // Track whether function uses interesting opcodes
    start; // JUMPDEST instruction offset
    jumps; // JUMPDEST instruction offsets this function can jump to
    end; // Last instruction offset before the next JUMPDEST
...
export class Program {
    dests; // instruction offset -> Function
    selectors; // function hash -> instruction offset
    notPayable; // instruction offset -> bytes offset
    fallback; // instruction offset for fallback function
    eventCandidates; // PUSH32 found before a LOG instruction
    proxySlots; // PUSH32 found that match known proxy slots
    proxies;
    init; // Program embedded as init code
    constructor(init) {
        this.dests = {};
        this.selectors = {};
        this.notPayable = {};
...
export function abiFromBytecode(bytecodeOrProgram) {
...
export function disasm(bytecode, config) {
    ...
    while (code.hasMore()) {
        ...
        if (inst === opcodes.DELEGATECALL &&
            ...
            else if (code.at(-3) === opcodes.SLOAD &&
        ...
        if (code.at(-3) === opcodes.EQ &&
            ...
            const selector = bytesToHex(value, 4);
